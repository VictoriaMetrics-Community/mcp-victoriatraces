...
var (
	toolTraces = mcp.NewTool(toolNameTraces,
		...
		mcp.WithNumber("start",
			...
			mcp.Description("Start timestamp in unix microseconds."),
		),
		mcp.WithNumber("end",
			mcp.Title("End timestamp"),
			mcp.Description("End timestamp in unix microseconds."),
		),
...
func toolTracesHandler(ctx context.Context, cfg *config.Config, tcr mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	operation, err := GetToolReqParam[string](tcr, "service", false)
	if err != nil {
		return mcp.NewToolResultError(err.Error()), nil
	}

	start, err := GetToolReqParam[float64](tcr, "start", true)
	if err != nil {
		return mcp.NewToolResultError(err.Error()), nil
	}
	...
	if start >= 0 {
		q.Add("start", fmt.Sprintf("%d", uint64(start)))
	}
	if end >= 0 {
		q.Add("end", fmt.Sprintf("%d", uint64(end)))
	}
	if minDuration != "" {
